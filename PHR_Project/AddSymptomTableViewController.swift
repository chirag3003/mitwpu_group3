...
class AddSymptomTableViewController: UITableViewController {
    ...
    func setupTypeMenu() {
        let options = ["Migraine", "Fatigue", "Dizziness", "Nausea", "Polyuria", "Blurred Vision", "Irritability", "Extreme Hunger", "Dry Mouth"]
        var actions: [UIAction] = []
        
        for option in options {
            actions.append(UIAction(title: option) { [weak self] action in
    ...
    @IBAction func saveButtonTapped(_ sender: Any)
    {
            guard let type = selectedType, let intensity = selectedIntensity else {
                // Show simple alert
                let alert = UIAlertController(title: "Missing Info", message: "Please select Type and Intensity", preferredStyle: .alert)
