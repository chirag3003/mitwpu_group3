...
class APIService {
    ...
    private func createDocumentMultipartBody(
        ...
        // documentType field
        body.append("--\(boundary)\(lineBreak)")
        body.append("Content-Disposition: form-data; name=\"documentType\"\(lineBreak)\(lineBreak)")
        body.append("\(documentType)\(lineBreak)")
        
        // docDoctorId field (for Prescription)
        if let doctorId = docDoctorId {
            body.append("--\(boundary)\(lineBreak)")
            body.append("Content-Disposition: form-data; name=\"docDoctorId\"\(lineBreak)\(lineBreak)")
