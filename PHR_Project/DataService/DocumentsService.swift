...
class DocumentService {
    ...
    func getDocumentsByDoctor(doctorId: String) -> [Document] {
        return documents.filter {
            $0.docDoctor?.apiID == doctorId || $0.docDoctorId == doctorId
        }
    }
    ...
    func fetchDocumentsFromAPI() {
        APIService.shared.request(endpoint: "/documents", method: .get) {
...
class PrescriptionService {
    ...
    func getAllPrescriptionData() -> [PrescriptionModel] {
        return DocumentService.shared.getPrescriptions().map {
            $0.asLegacyPrescriptionModel
        }
    }
